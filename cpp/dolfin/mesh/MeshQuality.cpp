...
std::array<double, 6> MeshQuality::dihedral_angles(const Cell& cell)
{
  ...
  for (std::uint32_t i = 0; i < 6; ++i)
  {
    ...
    const EigenPointVector p0 = Vertex(mesh, i0).point();
    EigenPointVector v1 = Vertex(mesh, i1).point() - p0;
    EigenPointVector v2 = Vertex(mesh, i2).point() - p0;
    EigenPointVector v3 = Vertex(mesh, i3).point() - p0;
    v1 /= v1.norm();
    v2 /= v2.norm();
    v3 /= v3.norm();
